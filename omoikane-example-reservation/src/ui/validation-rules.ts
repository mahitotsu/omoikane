...
export const emailFormatValidation: ValidationRule = {
  ...
  description:
    '有効なメールアドレス形式であることを検証します。RFC 5322準拠の基本的なパターンマッチングを行います。',
  ruleType: 'email',
...
export const phoneFormatValidation: ValidationRule = {
  ...
  type: 'validation-rule',
  description:
    '日本国内の電話番号形式を検証します。ハイフンあり（03-1234-5678）またはなし（09012345678）の両方に対応。',
  ruleType: 'tel',
  parameters: {
...
export const businessHoursValidation: ValidationRule = {
  ...
  type: 'validation-rule',
  description:
    '選択された時刻が店舗の営業時間内であることを検証します。営業時間は店舗設定に基づきます。',
...
export const partySizeRangeValidation: ValidationRule = {
  ...
  errorMessage:
    'ご利用人数は1名から10名の範囲で指定してください。10名を超える場合は店舗にお問い合わせください。',
  validateOn: ['blur', 'submit'],
};
...
export const cancellationDeadlineValidation: ValidationRule = {
  ...
  errorMessage:
    'キャンセルは予約時刻の24時間前までに行ってください。それ以降のキャンセルは店舗にお電話ください。',
  // TODO: ビジネスルール定義後に追加
