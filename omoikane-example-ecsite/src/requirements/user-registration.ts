...
export const customer: Actor = {
  ...
  role: 'primary',
  responsibilities: ['商品の閲覧・検索', 'アカウント登録・管理', '注文・決済', 'レビュー投稿'],
};
...
export const userRegistration: UseCase = {
  ...
  preconditions: ['顧客がECサイトにアクセスしている', 'インターネット接続が安定している'],
  postconditions: [
  ...
  alternativeFlows: [
    {
      ...
      steps: [
        ...
        },
        {
          ...
          actor: typedActorRef('customer'),
          action:
            '重複エラーメッセージを確認し、ログインページへの誘導または別メールアドレス入力を選択',
          expectedResult:
            '「このメールアドレスは既に登録されています。ログインしますか？」の選択肢が表示される',
        },
        {
