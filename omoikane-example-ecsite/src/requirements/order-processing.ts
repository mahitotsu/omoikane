...
export const shippingService: Actor = {
  ...
  role: 'external',
  responsibilities: ['配送料金計算', '配送日時予約', '配送状況追跡', '配送完了通知', '再配達対応'],
};
...
export const orderProcessing: UseCase = {
  ...
  mainFlow: [
    ...
    },
    {
      actor: typedActorRef('customer'),
      action:
        '決済方法（クレジットカード・電子マネー等）を選択し、決済情報を入力して「注文確定」をクリック',
...
export const orderTracking: UseCase = {
  ...
  alternativeFlows: [
    {
      ...
      steps: [
        ...
        },
        {
          ...
          action: '追跡不可メッセージを確認',
          expectedResult:
            '「追跡情報は一時的に取得できません。しばらく後に再度お試しください」というメッセージが表示',
        },
      ],
      returnToStepId: 'fetch-product-data',
    },
  ...
  businessValue:
    '顧客の不安を解消し、配送に関する問い合わせを削減。顧客満足度向上とカスタマーサポート負荷軽減',
  priority: 'medium',
