  ...
  alternativeFlows: [
    {
      ...
      steps: [
        ...
        },
        {
          ...
          expectedResult:
            '「メールアドレスまたはパスワードが正しくありません」というメッセージが表示',
        },
        {
          actor: typedActorRef('security-service'),
...
export const userLogout: UseCase = {
  ...
  actors: {
    primary: typedActorRef('customer'),
    secondary: [typedActorRef('database-system'), typedActorRef('security-service')],
  },
  preconditions: ['ユーザーがログイン状態', '有効なセッションが存在'],
  postconditions: [
    'ユーザーセッションが無効化される',
    'ログイン状態が解除される',
    'セキュリティログが記録される',
  ...
  alternativeFlows: [
    {
      ...
      steps: [
        ...
        },
        {
          ...
          expectedResult:
            '「セッションが期限切れです。再度ログインしてください」というメッセージが表示',
        },
      ],
  ...
  performanceRequirements: ['ログアウト処理: 1秒以内', 'セッション削除: 500ms以内'],
  uiRequirements: [
    'ログアウト確認ダイアログ（オプション）',
    'ログアウト完了メッセージ',
    'ログイン状態の視覚的表示更新',
