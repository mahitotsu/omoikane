 ...
 */
export function inferContext(projectName?: string, tags?: string[]): Partial<ProjectContext> {
  ...
  if (
    nameAndTags.includes('shop') ||
    nameAndTags.includes('cart') ||
    nameAndTags.includes('ecommerce')
  ) {
    inferred.domain = ProjectDomain.ECOMMERCE;
  } else if (
    nameAndTags.includes('bank') ||
    nameAndTags.includes('finance') ||
    nameAndTags.includes('payment')
  ) {
    inferred.domain = ProjectDomain.FINANCE;
  } else if (
    nameAndTags.includes('health') ||
    nameAndTags.includes('medical') ||
    nameAndTags.includes('hospital')
  ) {
    inferred.domain = ProjectDomain.HEALTHCARE;
  } else if (
    nameAndTags.includes('iot') ||
    nameAndTags.includes('device') ||
    nameAndTags.includes('sensor')
  ) {
    inferred.domain = ProjectDomain.IOT;
  } else if (
    nameAndTags.includes('analytics') ||
    nameAndTags.includes('data') ||
    nameAndTags.includes('ml')
  ) {
...
export function generateContextualRecommendations(
  context: ProjectContext,
  _applicationResult: ContextApplicationResult
): string[] {
  const recommendations: string[] = [];

  // ドメイン別の推奨
  const domainFocus = DomainRecommendedFocus[context.domain];
  recommendations.push(`🎯 ${context.domain}ドメイン: ${domainFocus.description}`);

  // ステージ別の推奨
  const stageFocus = StageRecommendedFocus[context.stage];
  recommendations.push(`📅 ${context.stage}ステージ: ${stageFocus.description}`);
  recommendations.push(`   推奨最低レベル: レベル${stageFocus.minLevel}`);

  // チーム規模別の推奨
  const teamFocus = TeamSizeRecommendedFocus[context.teamSize];
  recommendations.push(`👥 ${context.teamSize}チーム: ${teamFocus.description}`);

  // 重要度別の推奨
  const criticalityFocus = CriticalityRecommendedFocus[context.criticality];
  recommendations.push(`⚠️  ${context.criticality}重要度: ${criticalityFocus.description}`);
