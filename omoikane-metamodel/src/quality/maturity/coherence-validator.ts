 ...
 *
 * **検証内容:**
 * 1. 画面順序の一致性（循環パターンを考慮）
 * 2. 遷移の完全性（すべての遷移が定義されているか）
 * 3. 開始・終了画面の一致性
 *
 * **循環UIパターンのサポート:**
 * 「一覧 → 詳細 → 一覧」のような循環フローを正しく認識・検証します。
 * UseCaseで最後に最初の画面に戻る場合、ScreenFlowでは循環部分を除いた画面集合と比較します。
 *
 * **設計原則:**
 * - 型システムで保証できることはランタイム検証しない
 * - relatedUseCaseは必須フィールド（型で保証）
 * - 実世界のUIフロー（循環）を正しく評価
 *
 * **用途:**
 * - アーキテクチャ評価の一部として実行
 ...
 * 遷移の完全性を検証
 */
function validateTransitions(useCase: UseCase, screenFlow: ScreenFlow): CoherenceIssue[] {
  const issues: CoherenceIssue[] = [];
 ...
 * @returns 検証結果
 */
export function validatePrerequisiteUseCases(useCases: UseCase[]): CoherenceValidationResult {
  const issues: CoherenceIssue[] = [];
  const issuesByUseCase = new Map<string, CoherenceIssue[]>();
