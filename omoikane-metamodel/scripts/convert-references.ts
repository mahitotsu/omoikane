...
async function convertStringReferences() {
  ...
  for (const file of files) {
    ...
    updatedContent = updatedContent.replace(/primary:\s*'([^']+)'/g, (match, actorId) => {
      fileConversions++;
    ...
    updatedContent = updatedContent.replace(/actor:\s*'([^']+)'/g, (match, actorId) => {
      fileConversions++;
...
async function convertToStringReferences() {
  ...
  for (const file of files) {
    ...
    updatedContent = updatedContent.replace(/typedActorRef\('([^']+)'\)/g, (match, actorId) => {
      fileConversions++;
      return `'${actorId}'`;
    });
